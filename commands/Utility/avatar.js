...
module.exports = {
    ...
    execute(message, args) {
        (async () => {
            ...
            var te = new Discord.MessageEmbed().setTitle("Avatar").setDescription(`Avatar of <@!${user.id}>:`).setImage(`${avurl}.${fot}`).addField("Links", `[PNG](${avurl}.png) | [JPG](${avurl}.png) | [WEBP](${avurl}.webp)${ge}`).setColor('#0099FF').setAuthor({ name: user.tag, iconURL: avurl + '.' + fot }).setFooter({ text: 'We are not responsible for any content on the user\'s avatar.' });
